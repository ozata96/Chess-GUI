...
int ChessBoard::convertToRow(std::string coordinate){
        ...
        if (fromFileInt == 1)
            ...
            cerr << endl << endl << "Please enter coordinates in this format: " << endl;
...
bool ChessBoard::makeMove(ChessPiece *board[8][8], std::string fromPosition, std::string toPosition){
    ...
    if (current->isMoveLegal(board, rowFrom, colFrom, rowTo, colTo)){
        ...
        if (board[rowFrom][colFrom]->getColor() == White){
            cout << endl << "White's " << board[rowFrom][colFrom]->getType() << " cannot move from ";
            cout << fromPosition << " to " << toPosition << endl;
            return false;
        }
        if (board[rowFrom][colFrom]->getColor() == Black){
            cout << endl << "Black's " << board[rowFrom][colFrom]->getType() << " cannot move from ";
            cout << fromPosition << " to " << toPosition << endl;
            return false;
        }
    }
...
if (board[rowTo][colTo]->getColor() == White){
    ...
    if (isKingChecked(current->getColor())){
        if (current->getColor() == White){
            cout << endl << "Moving White's " << board[rowTo][colTo]->getType() << " from ";
    ...
    cout << endl << "White's " << board[rowTo][colTo]->getType() << " moves from ";
    ...
    if (!hasMoves(Black) && !isKingChecked(Black))
        cout << endl << "Game ends in stalemate - Black has no moves" << endl;
        
    if (isKingChecked(Black)){
            cout << endl << "Black is in check";
        if (!hasMoves(Black)){
            cout << "mate" << endl;
        }
    }
...
if (board[rowTo][colTo]->getColor() == Black){
    ...
    if (isKingChecked(current->getColor())){
        if (current->getColor() == Black){
            cout << endl << "Moving Black's " << board[rowTo][colTo]->getType() << " from ";
    ...
    cout << endl << "Black's " << board[rowTo][colTo]->getType() << " moves from ";
    cout << fromPosition << " to " << toPosition;
    changeTurns();
    if (piece != nullptr){
    ...
    if (!hasMoves(White) && !isKingChecked(White))
        cout << endl << "Game ends in stalemate - White has no moves" << endl;
    
    if (isKingChecked(White)){
            cout << endl << "White is in check";
        if (!hasMoves(White)){
            cout << "mate" << endl;
        }
    }
